...
def get_mnist_train_data():
    ...
    if len(train_data.shape) == 2:
        train_data = np.reshape(train_data, (train_data.shape[0], pic_w, pic_h, 1))
...
def get_mnist_test_data():
    ...
    if len(test_data.shape) == 2:
        test_data = np.reshape(test_data, (test_data.shape[0], pic_w, pic_h, 1))
...
def get_mnist_validate_data():
    ...
    if len(validate_data.shape) == 2:
        validate_data = np.reshape(validate_data, (validate_data.shape[0], pic_w, pic_h, 1))
...
def get_standard_train_data(name):
    data, label = mnist_reader.load_mnist(fashionPath, kind='train')
    if len(data.shape) == 2:
        train_data = np.reshape(data, (data.shape[0], pic_w, pic_h, 1))
...
def get_standard_validate_data(name):
    data, label = mnist_reader.load_mnist(fashionPath, kind='t10k')
    if len(data.shape) == 2:
        train_data = np.reshape(data, (data.shape[0], pic_w, pic_h, 1))
...
def get_train_data(batch_size):
    ...
    image_batch, label_batch = tf.train.batch([single_image, single_label], batch_size=batch_size, num_threads=2,
